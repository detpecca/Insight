...
public class AiOpsService {
    ...
    private static final String TASK_PROMPT = "你是企业级 SRE，接到了自动化告警排查任务。请结合工具调用，" +
            "执行**规划→执行→再规划**的闭环，并最终按照固定模板输出《告警分析报告》。" +
    ...
    /** AIOps 专用模型（temperature 0.3 / maxToken 8000），见 DashScopeConfig */
    @Autowired
    @Qualifier("aiOpsChatModel")
    private DashScopeChatModel aiOpsChatModel;

    /**
     * 以流式方式执行 AI Ops 告警分析流程。
     * 每个节点（Planner/Executor）完成时都会发出 {@link NodeOutput}，
     * 调用方可将其转换为进度事件实时推送给前端，而不必干等整个编排结束。
     *
     * @param toolCallbacks MCP 提供的工具回调数组
     * @return 节点输出流；流末尾的 {@link OverAllState} 含最终状态（blockLast 获取）
     */
    public Flux<NodeOutput> streamAiOpsAnalysis(ToolCallback[] toolCallbacks) throws GraphRunnerException {
        logger.info("开始执行 AI Ops 多 Agent 协作流程（流式）");

        SupervisorAgent supervisorAgent = buildSupervisorAgent(toolCallbacks);

        logger.info("调用 Supervisor Agent 开始编排...");
        return supervisorAgent.stream(TASK_PROMPT);
    }
    ...
    /**
     * 构建 Supervisor Agent（编排 Planner 与 Executor）
     */
    private SupervisorAgent buildSupervisorAgent(ToolCallback[] toolCallbacks) {
     ...
     * 注：AIOps Agent 不注入 MemoryTools —— 自动分析流程不应改写全局规则库
     */
    ...
    private String buildPlannerPrompt() {
        return """
                ...
                <summary>这里是一段不超过50个字的，关于本次故障分析结论的精准摘要</summary>
                ...
                **重要提醒**：
                ...
                - 直接从 "# 告警分析报告" 开始输出，不要遗漏下方的 <summary> 标签
    ...
    private String buildSupervisorSystemPrompt() {
        return """
                ...
                4. FINISH 后，确保向最终用户输出完整的《告警分析报告》，格式必须严格要求在标题下方紧跟 XML 标签，即：
                   # 告警分析报告\n<summary>精准摘要</summary>\n---\n## 活跃告警清单\n## 告警根因分析N...
